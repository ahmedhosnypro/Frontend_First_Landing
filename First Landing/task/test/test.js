...
class Test extends StageTest {
    ...
    tests = [this.node.execute(async () => {
        ...
        this.page.execute(() => {
            ...
            // check if header has inner text
            let errorMsg = "The header tag is missing a placeholder inner text.";
            if (this.innerTextExist("header")) return wrong(errorMsg)

            // check if main has inner text
            errorMsg = "The main tag is missing a placeholder inner text.";
            ...
            // check if header has bg color
            let errorMsg = "The header tag doesn't have the background color value.";
            if (this.bgColorExist("header")) return wrong(errorMsg)

            // check if main has bg color
            errorMsg = "The main tag doesn't have the background color value.";
            if (this.bgColorExist("main")) return wrong(errorMsg)

            // check if footer has bg color
            errorMsg = "The footer tag doesn't have the background color value.";
            if (this.bgColorExist("footer")) return wrong(errorMsg)

            // check if these 3 elements have a different bg color from each other
            errorMsg = "The 3 elements don't have different background colors from each other.";
            const getBackgroundColor = (node) => {
                const element = document.querySelector(node);
                return getComputedStyle(element).backgroundColor;
            };
            const elementColors = [getBackgroundColor("header"), getBackgroundColor("main"),
                getBackgroundColor("footer")];
            const reduced = elementColors.reduce((acc, item) => {
                acc[item] = acc[item] ? acc[item] + 1: 1;
                return acc;
            }, {});
            const resultCheck = Object.values(reduced).find(item => item > 1);
            if (resultCheck) return wrong(errorMsg);
